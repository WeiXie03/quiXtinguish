...
gst-launch-1.0 -v v4l2src device=/dev/video0 ! video/x-raw,format=YUY2,width=960,height=720 ! jpegenc ! rtpjpegpay ! udpsink host=$HOST port=5000 &
gst-launch-1.0 -v v4l2src device=/dev/video1 ! video/x-raw,format=YUY2,width=960,height=720 ! jpegenc ! rtpjpegpay ! udpsink host=$HOST port=5200
