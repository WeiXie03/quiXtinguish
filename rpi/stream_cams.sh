...
gst-launch-1.0 -v v4l2src device=/dev/video0 ! video/x-raw, format=YUY2, width=640, height=480, bitrate=1900000, framerate=20/1 ! jpegenc ! rtpjpegpay ! udpsink host=$HOST port=$LEFT_PORT &
gst-launch-1.0 -v v4l2src device=/dev/video2 ! video/x-raw, format=YUY2, width=640, height=480, bitrate=1900000, framerate=20/1 ! jpegenc ! rtpjpegpay ! udpsink host=$HOST port=$RIGHT_PORT &
...
#raspivid -n -t 0 -b 2505000 -fps 20 -w 640 -h 480 -o - | gst-launch-1.0 -v fdsrc ! h264parse ! rtph264pay pt=96 config-interval=5 ! udpsink host=$HOST port=$PICAM_PORT
raspivid -n -t 0 -b 2600000 -fps 20 -w 640 -h 480 -cd MJPEG -o - | gst-launch-1.0 -v fdsrc ! "image/jpeg,framerate=20/1" ! jpegparse ! rtpjpegpay ! udpsink host=$HOST port=$PICAM_PORT
