...
gst-launch-1.0 -v v4l2src device=/dev/video1 ! video/x-raw, format=YUY2, width=640, height=480, bitrate=2500000, framerate=20/1 ! jpegenc ! rtpjpegpay ! udpsink host=$HOST port=$LEFT_PORT &
gst-launch-1.0 -v v4l2src device=/dev/video5 ! video/x-raw, format=YUY2, width=640, height=480, bitrate=2500000, framerate=20/1 ! jpegenc ! rtpjpegpay ! udpsink host=$HOST port=$RIGHT_PORT
...
#raspivid -n -t 0 -b 2500000 -fps 20 -w 640 -h 480 -cd MJPEG  -o - | gst-launch-1.0 -v fdsrc ! jpegparse ! rtpjpegpay ! udpsink host=$HOST port=$PICAM_PORT
