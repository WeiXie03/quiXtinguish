...
if __name__ == "__main__":
    ...
    #for imNum in range(76, len(metadata.keys())):
        ...
        for src in (left, right):
            ...
            src.img = cv2.imread(img_path)

        calib_dat = np.load('calibration/calib_settings/{}/calibSettings_{}.npy'.format(nir.side, nir.side))
        new_mtx = np.load('calibration/calib_settings/{}/camMtx_{}.npy'.format(nir.side, nir.side))
        nir.img = cv2.undistort(cv2.imread(paird[nir.side]['img_path']), calib_dat[0], calib_dat[1], None, new_mtx) #get img_path from metadata
