...
class Cam():
    def __init__(self, port, name, make_win=True):
        self.name = name
        #self.cap = cv2.VideoCapture('udpsrc port={} ! application/x-rtp,media=video,payload=26,encoding-name=JPEG ! rtpjpegdepay ! jpegdec ! videoconvert ! appsink'.format(port), cv2.CAP_GSTREAMER)
...
if __name__ == "__main__":
    DATA_DIR = 'data/nir_test/'
    print('Setting up cams')
    stream = Cam(5000, 'NoIR')
